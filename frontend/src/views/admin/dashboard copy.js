...
//     console.log('getDocumentStatus called with userId:', userId);
...
//     if (!userId) {
//       console.log('No userId provided, returning default status');
//       return {
...
//       const isValid = doc && doc.user_id === userId && doc.status !== 'deleted';
//       if (!isValid) {
//         console.log('Filtered out document:', doc);
//       }
//       return isValid;
//     });
...
//       console.log(`Status for ${type}:`, doc);
...
//       console.log(`Final status for ${type}:`, status);
//       return status;
//     };
...
//     console.log('Final document status result:', result);
//     return result;
//   };

//   // Adăugăm o funcție helper pentru a verifica statusul documentului
...
//   // Adăugăm o funcție helper pentru a verifica dacă documentul există
...
//         updateSectionData('status', response.data.status);
//       }

//       if (response.data.total) {
//         updateSectionData('total', response.data.total);
//       }
...
//   const handleDocumentDeleteSuccess = async (deletedDocument) => {
//     try {
//       // Reîncărcăm lista de documente după ștergere
//       await loadDocuments();
      
//       // Actualizăm starea locală
//       setDocuments(prevDocuments => 
//         prevDocuments.filter(doc => doc.id !== deletedDocument.id)
//       );
...
//     } catch (error) {
//       console.error('Eroare la actualizarea listei de documente după ștergere:', error);
//       setError('A apărut o eroare la actualizarea listei de documente. Vă rugăm să reîncărcați pagina.');
//       setTimeout(() => setError(null), 5000);
//     }
//   };
...
//           setError(error.response.data.message || 'Documentul nu poate fi șters deoarece este asociat cu o aplicație activă');
//         } else {
...
//               <button onClick={() => handleConfirmDocument(doc)}>Confirmă</button>
//               <button onClick={() => handleRejectDocument(doc)}>Respinge</button>
//             </div>
//           )}
//         </div>
//       );
...
//                         <span className="detail-value">{new Date(selectedApplication.createdAt).toLocaleDateString('ro-RO')}</span>
//                       </div>
//                       <div className="detail-row">
...
//                            selectedApplication.status === 'withdrawn' ? 'Retrasă' :
