...
router.post('/register', async (req, res) => {
  try {
    console.log('=== Începe procesul de înregistrare ===');
    ...
    if (!name || !email || !password) {
      console.log('Câmpuri lipsă:', { name: !!name, email: !!email, password: !!password });
    ...
    // Validare lungime minimă pentru parolă
    if (password.length < 3) {
      console.log('Parolă prea scurtă:', password.length);
      return res.status(400).json({
        success: false,
        message: 'Parola trebuie să aibă cel puțin 3 caractere'
      });
    }

    // Validare lungime minimă pentru nume
    if (name.length < 2) {
      console.log('Nume prea scurt:', name.length);
      return res.status(400).json({
        success: false,
    ...
    if (!result.success) {
      console.log('Înregistrare eșuată:', result.message);
      return res.status(400).json({
        success: false,
    ...
    console.log('Înregistrare reușită pentru:', email);
    res.json({
      success: true,
